...
# Main keys are 1) clipped double q learning, 2) delayed policy update, and 3) target policy smoothing based on DDPG.
import torch
...
class TD3():
    ...
    def learn(self):
        ...
        # Get the next action with noise
        next_actions = self.actor_target_net(next_states) + noise
        next_actions = next_actions.clamp(min=self.action_min, max=self.action_max)
        
        # Get the target q values through clipped double q
        target_q1, target_q2 = self.critic_target_net(next_states, next_actions)
        target_q = torch.min(target_q1, target_q2)
        target_q = (rewards + self.gamma * target_q * (1-dones))

        # Current Q values
        values1, values2 = self.critic_net(states, actions)

        # Calculate the critic loss and optimize the critic network
        critic_loss = F.mse_loss(values1, target_q.detach()) + F.mse_loss(values2, target_q.detach())
...
def main():
    ...
    agent = TD3(env, memory_size=100000, batch_size=64, gamma=0.95, learning_rate=1e-3, eps_min=0.00001, eps_period=100000, tau=0.005, policy_noise=0.2, noise_clip=0.5, policy_freq=2)
